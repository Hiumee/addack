...
func (db *Database) CreateChallengesTable() error {
	_, err := db.DB.Exec("CREATE TABLE IF NOT EXISTS challenges (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, name TEXT, command TEXT, path TEXT)")
	return err
}

// CreateChallenge creates a new challenge in the database
func (db *Database) CreateChallenge(challenge model.Challenge) (int64, error) {
	res, err := db.DB.Exec("INSERT INTO challenges (name, command, path) VALUES ($1, $2, $3)", challenge.Name, challenge.Command, challenge.Path)
...
func (db *Database) GetChallenge(id int64) (model.Challenge, error) {
	...
	err := db.DB.QueryRow("SELECT * FROM challenges WHERE id = $1", id).Scan(&challenge.Id, &challenge.Name, &challenge.Command, &challenge.Path)
...
func (db *Database) GetChallenges() ([]model.Challenge, error) {
	...
	for rows.Next() {
		var challenge model.Challenge
		err := rows.Scan(&challenge.Id, &challenge.Name, &challenge.Command, &challenge.Path)
		if err != nil {
			return challenges, err
		}
...
func (db *Database) UpdateChallenge(challenge model.Challenge) error {
	_, err := db.DB.Exec("UPDATE challenges SET name = $1, command = $2, path = $3 WHERE id = $4", challenge.Name, challenge.Command, challenge.Path, challenge.Id)
	return err
}
